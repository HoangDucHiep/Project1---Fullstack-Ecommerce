...
public sealed class GetProductBySlugQueryHandler : IQueryHandler<GetProductBySlugQuery, ProductDetailDto>
{
    ...
    {
        Product? product = await _productRepository.GetBySlugAsync(request.Slug, cancellationToken);
        if (product == null)
        {
            return Result.Failure<ProductDetailDto>(ProductErrors.NotFound(request.Slug));
        }
